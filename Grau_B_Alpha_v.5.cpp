...
while (menu = true) {
    ...
    cout<<"Menu de opções:\n 1) Agendar um Ingresso \n 2) Consultar Situação do Assento \n 3) Sair \n ";
    cin>>choice;
    
    while (choice != 1 && choice != 2 && choice != 3) { "Não existe essa opção, lembre-se que: \n 1) Agendar um Ingresso \n 2) Consultar Situação do Assento \n 3) Sair \n "; cin>>choice; }
...
if (choice == 1) {
    ...
    if (ocupante.sexo == 'f' || ocupante.sexo == 'F') {
        
        CSF++;
        
    } else if (ocupante.sexo == 'm' || ocupante.sexo =='M') {
        
        CSM++;

    }
    
        if (ocupante.idade < 18 || ocupante.idade > 59) {
